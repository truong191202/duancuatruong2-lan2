	...
	if(notChange){
		...
		for(let i = 0; i < listAccount.length; i++){
			if(inputForm[0].value == listAccount[i].username && inputForm[1].value == listAccount[i].password){
				...
				localStorage.setItem("nameUser", nameUser.innerHTML)
			}
			else{
				exitAll();
...
if(isLogin){
	...
	avatar.style.display = "block"
	nameUser.style.display = "inline"
	nameUser.innerHTML = localStorage.getItem("nameUser")
}
...
// Truyen cac gia tri cua san pham xem chi tiet vao localStorage
const localStorageSet = function(){
	...
	localStorage.setItem("viewInfoTitle", viewInfoTitle);
	localStorage.setItem("viewInfoPrice", viewInfoPrice);
	localStorage.setItem("viewInfoImage", viewInfoImage)
}

// Truyen cac gia tri san pham xem chi tiet sang gio hang sau khi bam nut them vao gio hang
const addProduct = $(".viewinfo .btn2")
let parse = localStorage.getItem("productCurrentArrayStringify")
...
addProduct.onclick = () => {
	...
	localStorage.setItem("productStringify", productStringify);
	productCurrentArrayStringify = JSON.stringify(productCurrentArray)
	localStorage.setItem("productCurrentArrayStringify", productCurrentArrayStringify)	
}
